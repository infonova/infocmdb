...
class Util_Search_Document_Docx extends Zend_Search_Lucene_Document_Docx
{
    ...
    public function __construct($values, $documentName)
    {
        ...
        $res = $this->loadDocxFile($documentName, true);

        if ($res->getFieldValue('creator'))
            $values['Author'] = $res->getFieldValue('creator');

        if ($res->getFieldValue('created'))
            $values['CreationDate'] = $res->getFieldValue('created');

        if ($res->getFieldValue('modified'))
            $values['ModDate'] = $res->getFieldValue('modified');

        if ($res->getFieldValue('body'))
