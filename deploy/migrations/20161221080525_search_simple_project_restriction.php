...
class SearchSimpleProjectRestriction extends AbstractPhinxMigration
{
    /**
     * Migrate Up.
     */
...
BEGIN
  ...
  declare search_limit        integer DEFAULT 1000;
  ...
  IF project_list IS NOT NULL then
      set @query = concat(
          'DELETE FROM " . $this->dbName . ".search_result
          WHERE " . $this->dbName . ".search_result.session = ', session_id,'
          AND " . $this->dbName . ".search_result.ci_id NOT IN ( SELECT p.ci_id FROM " . $this->dbName . ".ci_project p where p.project_id in (',project_list,'));');
