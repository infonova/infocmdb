...
class WorkflowAddStatus extends AbstractPhinxMigration{
    ...
    public function up() {
        ...
        if($workflowTable -> hasColumn('status')===false){
            //Status-Icon
            $this->execute("ALTER TABLE `" . $this->dbName . "_tables`.`workflow` ADD COLUMN `status` ENUM('0','1') NULL AFTER `execution_time`");
            $this->execute("ALTER TABLE `" . $this->dbName . "_history`.`workflow` ADD COLUMN `status` ENUM('0','1') NULL AFTER `execution_time`");
            //Status-Message
            $this->execute("ALTER TABLE `" . $this->dbName . "_tables`.`workflow` ADD COLUMN `status_message` TEXT NULL AFTER `status`");
            $this->execute("ALTER TABLE `" . $this->dbName . "_history`.`workflow` ADD COLUMN `status_message` TEXT NULL AFTER `status`");
            ...
            $this->execute("
                ...
                BEGIN          
                    ...
                    (NEW.response_format IS NOT NULL AND STRCMP(NEW.response_format, OLD.response_format) <> 0)
                    THEN SET changed = 1;
                    END IF;
                    
                    IF changed > 0 THEN
                        INSERT INTO ". $this->dbName ."_history.workflow(id, name, description, note, execute_user_id, is_async, trigger_ci, trigger_attribute, trigger_project, trigger_relation, trigger_time, execution_time, status, status_message, is_active, user_id, valid_from, response_format, user_id_delete, valid_to)
                        VALUES(OLD.id, OLD.name, OLD.description, OLD.note, OLD.execute_user_id, OLD.is_async, OLD.trigger_ci, OLD.trigger_attribute, OLD.trigger_project, OLD.trigger_relation, OLD.trigger_time, OLD.execution_time, OLD.status, OLD.status_message, OLD.is_active, OLD.user_id, OLD.valid_from, OLD.response_format, NEW.user_id, NOW());
            ...
            $this->execute("
                ...
                BEGIN
                    INSERT INTO ". $this->dbName ."_history.workflow(id, name, description, note, execute_user_id, is_async, trigger_ci, trigger_attribute, trigger_project, trigger_relation, trigger_time, execution_time, status, status_message, is_active, user_id, valid_from, response_format, user_id_delete, valid_to)
                    VALUES(OLD.id, OLD.name, OLD.description, OLD.note, OLD.execute_user_id, OLD.is_async, OLD.trigger_ci, OLD.trigger_attribute, OLD.trigger_project, OLD.trigger_relation, OLD.trigger_time, OLD.execution_time, OLD.status, OLD.status_message, OLD.is_active, OLD.user_id, OLD.valid_from, OLD.response_format, '0', NOW());
                END;
            ");
        }
