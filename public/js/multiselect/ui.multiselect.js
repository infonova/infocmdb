...
var defaultNodeComparator = function(node1,node2) {
	...
	{
		return 0;
	} else if(text1 % 1 === 0 && text2 % 1 === 0)
	{
		return ((text1 - text2) < 0? -1 : 1);
	}
...
$.widget("ui.multiselect", {
  options: {
	...
	_create: function() {
		...
		this.selectedActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="count">'+$.tmpl($.ui.multiselect.locale.itemsCount,{count:0})+'</span></div>').appendTo(this.selectedContainer);
		this.availableActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="busy">'+$.tmpl($.ui.multiselect.locale.busy)+'</span><input type="text" class="search ui-widget-content ui-corner-all"/></div>').appendTo(this.availableContainer);
		...
		if(elementId == '') {
			elementId = 'ui-multiselect-' + Math.floor((Math.random() * 10000) + 1);
			this.element.attr('id', elementId);
		}
		this.container.attr('data-select-id', elementId);
		...
		this.availableActions.find('input').width(Math.max(this.availableActions.width() - this.availableActions.find('a.add-all').width() - 30, 150));
		// fix list height to match <option> depending on their individual header's heights
		this.selectedList.height(Math.max(this.element.height()-this.selectedActions.height(),1));
		this.availableList.height(Math.max(this.element.height()-this.availableActions.height(),1));
        if(this.options.itemIncreasable == true) {
            this.maxOptionWidth = this.selectedList[0].clientWidth - 80;
        } else {
            this.maxOptionWidth = this.selectedList[0].clientWidth - 50;
        }
	...
	addOptions: function(data) {
		...
		if (this.enabled()) {
			...
			if (data = this.options.dataParser.call(this, data)) {
				that.availableList.children('li.ui-element').remove();
				that.availableList.data('multiselect.cache', {});
				that.element.find('option:not(:selected)').remove();
				...
				$.each(data, function(index, dataRow) {
					...
					var selected = false;
					if(key.split("_")[0] == "s"){
						cleanedKey = key.split("_")[1];
						selected = true;
					}

					var keyParts = cleanedKey.split("::");
					var cleanedKeyWithoutAmount = keyParts[0];
					var amount = keyParts[1];
					...
					if (that.element.find('option[data-value="'+cleanedKeyWithoutAmount+'"]').length === 0) {
						if(cleanedKeyWithoutAmount !== '')
						{
							var optionElement = $('<option value="' + cleanedKey + '" data-value="' + cleanedKeyWithoutAmount + '" data-amount="' + amount + '" />');
							if (selected === true) {
								optionElement.prop('selected', true);
							}
							elements.push(optionElement.text(dataRow.value).appendTo(that.element)[0]);
						}
					} else {
						// update option description
						that.element.find('option[data-value="'+cleanedKeyWithoutAmount+'"]').text(dataRow.value);
						that.container.find('div.list-container > ul.list > li.ui-element').each(function() {
							if($(this).data('multiselect.optionLink').attr('data-value') == cleanedKeyWithoutAmount) {
								var optionText = that._prepareOptionText(dataRow.value);
								$(this).find('div.option-text').html(optionText);
							}
						});
					}
				});
    ...
    **************************************/
			...
			list.sortable({
				...
				receive: function (event, ui) {
					...
					if (opts[otherSide].sortable) {
						...
						ui.item.hide();
						if(side == 'available') {
							that._setSelected(ui.item, itemSelected, true, 0);
						} else {
					...
					// bug with jQuery Upgrade from 1.4 to 1.9 -> on drag'n'drop div.option-text is shown for each item
					//$(event.target).children('.option-text').hide();
				},
	...
	_prepareOptionText: function(text) {
		return text;
	},
	_populateLists: function(options) {
		...
		$(options.each(function(i) {
			...
			var optionData = optionValue.split('::');

			option.attr('data-value', optionData[0]);
			if(typeof(optionData[1]) === 'undefined') {
				if(this.selected) {
					option.attr('data-amount', 1);
				} else {
					option.attr('data-amount', 0);
				}
			} else {
				option.attr('data-amount', optionData[1]);
			}

			if(that.options.itemIncreasable) {
				option.val(option.attr('data-value') + '::' + option.attr('data-amount'));
			}
	...
	_getOptionNode: function(option) {
		option = $(option);
		var optionText = this._prepareOptionText(option.text());

		// minus input plus
        var node = $('<li class="ui-state-default ui-element"><div class="option-text" style="width: '+this.maxOptionWidth+'px">'+optionText+'</div><div style="float: left; padding-right: 5px;" class="no-drag"><a href="#" class="ui-state-default action remove-action no-drag"><span class="ui-corner-all ui-icon ui-icon-minus no-drag"/></a><input type="text" value="0" style="width: 25px" class="no-drag" /><a href="#" class="ui-state-default action add-action no-drag"><span class="ui-corner-all ui-icon ui-icon-plus no-drag"/></a></div></li>').hide();
   ...
   // @param bool noclone (optional) true only if item should not be cloned on the other list
	_setSelected: function(item, selected, noclone, counterValue) {
		var that = this, otherItem;
		var optionLink = item.data('multiselect.optionLink');
		var optionCounter = parseInt(optionLink.attr('data-amount'));
		if(counterValue != undefined) {
			optionCounter = counterValue;
		} else {
			if (selected) {
				optionCounter++;
			} else {
		...
		item.find('input').val(optionCounter);
		optionLink.attr('data-amount', optionCounter);
		if(this.options.itemIncreasable) {
			optionLink.val(optionLink.attr('data-value') + '::' + optionCounter);
		}
		...
		if(otherItem) {
			otherItem.find('input').val(optionCounter); // for some reason the value needs to be set after show event again?!
		}


		if (!this.busy && otherItem) {
			if (this.options.animated) {
				// pulse
	...
	_applyItemState: function(item, selected) {
		if(this.options.itemIncreasable == false) {
		...
		if(selected) {
			...
			if(this.options.itemIncreasable == false) {
				item.find('a.add-action').hide();
				item.find('input').hide();
			}
			item.find('a.remove-action').show();
			...
			item.find('input').hide();
			item.find('a.remove-action').hide();
			item.find('a.add-action').show();
		}
		this._registerRemoveEvents(item.find('a.remove-action'));
		this._registerAddEvents(item.find('a.add-action'));
		this._registerChangeAmountEvents(item.find('input'));
	...
	_registerAddEvents: function(elements) {
		...
		elements.unbind('click.multiselect').bind('click.multiselect', function() {
			that._setSelected($(this).parent().parent(), true);
			return false;
		});
	...
	_registerRemoveEvents: function(elements) {
		var that = this;
		elements.unbind('click.multiselect').bind('click.multiselect', function() {
			that._setSelected($(this).parent().parent(), false);
			return false;
		});
	...
	_registerChangeAmountEvents: function(elements) {
		var that = this;

		elements.unbind('change.multiselectCounter').bind('change.multiselectCounter', function() {
			var optionCounter = $(this).val();
			var selected = false;
			if(optionCounter > 0) {
				selected = true;
			}
			that._setSelected($(this).parent().parent(), selected, false, optionCounter);
		});

		elements.unbind('keypress.multiselectCounter').bind('keypress.multiselectCounter', function(e) {
		...
		var _searchNow = function(forceUpdate) {
			...
			if ((value != previousValue) || (forceUpdate)) {
				...
				if (that.options.remoteUrl) {
					that.container.addClass('ui-widget-loading');
					...
					try {					
						$.ajax({
							...
							success: function(data) { 
								...
								that._setBusy(false);
								that.container.removeClass('ui-widget-loading');
							},
...
$.extend($.ui.multiselect, {
	...
	locale: {
		...
		itemsCount:'ausgewählte Einträge',
